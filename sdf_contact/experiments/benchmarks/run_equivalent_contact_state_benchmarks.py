...
OUT = ROOT / 'results' / 'equivalent_contact_state_benchmarks'
...
def make_solver():
    cfg = IntegratorConfig(dt=DT, collect_diagnostics=True, onset_focus_enabled=True)
    cfg.early_active_interval_report_force = False
    cfg.equivalent_contact_state_report_force = False
    cfg.onset_microstep_enabled = False
    cfg.early_active_interval_samples = 1
    cfg.onset_bisect_iters = 3
    cfg.onset_local_refine_steps = 0
...
def _run_until_first_contact(world, solver, delta_fn, static_force_fn, max_steps=20):
    ...
    for step in range(max_steps):
        ...
        if Fy_end > 1.0e-8 and result is None:
            ...
            for key in ('onset_aligned', 'onset_microstep', 'onset_equivalent_state', 'onset_impulse_matched_state'):
...
def _plot(df, title, out_png):
    ...
    styles = {
        ...
        'onset_equivalent_state': ('s', 'equivalent-state static Fy'),
        'onset_impulse_matched_state': ('^', 'impulse-matched static Fy'),
    }
...
def main():
    ...
    R = 0.12
    h0 = 0.02
    sphere_df, sphere_res = _run_until_first_contact(_sphere_world(R, h0), make_solver(), lambda y: R - y, lambda d: K_EQ * cap_volume(R, d), max_steps=20)
    sphere_df.to_csv(OUT / 'sphere_first_contact.csv', index=False)
    _plot(sphere_df, 'Sphere first-contact equivalent-state benchmark', OUT / 'sphere_first_contact.png')
    ...
    h0 = 0.01
    flat_df, flat_res = _run_until_first_contact(_box_world(ext, h0), make_solver(), lambda y: 0.5 * ext[1] - y, lambda d: area * K_EQ * d, max_steps=14)
    flat_df.to_csv(OUT / 'flat_first_contact.csv', index=False)
    _plot(flat_df, 'Flat-punch first-contact equivalent-state benchmark', OUT / 'flat_first_contact.png')
    rows.append({'case': 'flat_punch_centered', **(flat_res or {})})

    overview = pd.DataFrame(rows)
    overview.to_csv(OUT / 'comparison.csv', index=False)
    lines = ['# Equivalent contact state first-contact benchmarks', '']
    lines.append('This benchmark compares first-contact force samples from the coarse step end, onset-aligned state, onset micro-step, and a new equivalent contact state that uses the midpoint active-interval state but re-evaluates contact with the static high-accuracy evaluator.')
    lines.append('')
    for _, row in overview.iterrows():
        lines.append(f"## {row['case']}")
        lines.append(f"- step_end_rel_err = {row['step_end_rel_err']:.6f}")
        for prefix in ('onset_aligned', 'onset_microstep', 'onset_equivalent_state', 'onset_impulse_matched_state'):
