...
# -*- coding: utf-8 -*-
"""
implicit_contact_framework_v4.py

Upgrade: ContactManager is now a true multi-body pair manager.

Supported pair families:
    1) body-domain pairs (rigid body vs fixed compliant cube domain)
       using the unified 4-stage pipeline:
           build_contact_patches -> extract_sheet -> integrate_tractions -> assemble_wrench

    2) body-body sphere-sphere pairs
       using a direct point-contact normal law
       (kept intentionally simple and explicit in geometry support)

Important honesty note:
    - This file upgrades CONTACT MANAGEMENT to true multi-body pair enumeration and aggregation.
    - The per-body solver still treats other bodies as frozen during one body's Newton solve
      (block-style implicit solve), not a single monolithic all-body Newton system.
    - So this is the correct next architecture step for the manager, while remaining practical.
"""
...
class ContactModelConfig:
    ...
    top_y: float = 0.0
    sphere_sphere_stiffness: float = 20000.0
    sphere_sphere_damping: float = 150.0
...
class ContactManager:
    ...
    def _compute_body_domain_pair(self, body_i: int, body: RigidBody6D, domain: DomainSpec) -> Optional[PairRecord]:
        ...
        if np.linalg.norm(wrench.force) <= 1.0e-14 and len(sheet.samples) == 0:
            return None
        contrib_i = PairWrenchContribution("body_domain", wrench.force.copy(), wrench.moment.copy(), {
    ...
    def compute_all_contacts(self, world: World) -> list[AggregatedBodyContact]:
        out = [AggregatedBodyContact(np.zeros(3), np.zeros(3), [], 0, 0, 0) for _ in world.bodies]
...
def accumulate_body_forces(body_index: int, world: World, contact_manager: ContactManager) -> ForceAssemblyResult:
    contacts = contact_manager.compute_all_contacts(world)
    c = contacts[body_index]
...
class ImplicitSystemSolver6D:
    ...
    def _make_trial_world(self, world: World, body_index: int, u: np.ndarray) -> World:
        dt = self.cfg.dt
        new_bodies: list[RigidBody6D] = []
        for k, body in enumerate(world.bodies):
            if k != body_index:
            ...
            new_state = BodyState6D(Pose6D(x, q), v.copy(), w.copy())
    ...
    def _eval_residual(self, world: World, body_index: int, u: np.ndarray):
        body0 = world.bodies[body_index]
        v0 = body0.state.linear_velocity
        w0 = body0.state.angular_velocity
        trial_world = self._make_trial_world(world, body_index, u)
        trial_body = trial_world.bodies[body_index]
        forces = accumulate_body_forces(body_index, trial_world, self.contact_manager)
        v = trial_body.state.linear_velocity
        w = trial_body.state.angular_velocity
        Iw = trial_body.inertia.inertia_world(trial_body.state.pose.orientation)
        Rv = trial_body.inertia.mass * (v - v0) - self.cfg.dt * forces.total_force
        Rw = Iw @ (w - w0) + self.cfg.dt * np.cross(w, Iw @ w) - self.cfg.dt * forces.total_moment
        return np.concatenate([Rv, Rw]), forces

    def step_body(self, world: World, body_index: int) -> dict:
        body = world.bodies[body_index]
        if body.is_static:
            return {"contact_force": np.zeros(3), "contact_moment": np.zeros(3), "num_projected_points": 0, "num_sheet_points": 0, "num_tractions": 0, "num_pairs": 0}
        u = np.concatenate([body.state.linear_velocity, body.state.angular_velocity]).astype(float)
        for _ in range(self.cfg.newton_max_iter):
            R, _ = self._eval_residual(world, body_index, u)
            ...
            for k in range(6):
                up = u.copy(); up[k] += eps
                Rp, _ = self._eval_residual(world, body_index, up)
                J[:, k] = (Rp - R) / eps
            try:
            ...
            for alpha in self.cfg.line_search_factors:
                un = u + alpha * du
                Rn, _ = self._eval_residual(world, body_index, un)
        ...
        v = u[:3]
        w = u[3:]
        body.state.pose.position = body.state.pose.position + dt * v
        body.state.pose.orientation = integrate_quaternion(body.state.pose.orientation, w, dt)
        body.state.linear_velocity = v
        body.state.angular_velocity = w
        forces = accumulate_body_forces(body_index, world, self.contact_manager)
        return {
            "contact_force": forces.contact_force.copy(),
            "contact_moment": forces.contact_moment.copy(),
            "num_projected_points": forces.contact_meta["num_projected_points"],
            "num_sheet_points": forces.contact_meta["num_sheet_points"],
            "num_tractions": forces.contact_meta["num_tractions"],
            "num_pairs": forces.contact_meta["num_pairs"],
        }
...
class Simulator:
    ...
    def step(self) -> None:
        ...
        for i, body in enumerate(self.world.bodies):
            if body.is_static:
                continue
            info = self.solver.step_body(self.world, i)
            updated = self.world.bodies[i]
