...
\end{equation}
因此，这一组算例可以同时检验“力是否正确”和“作用点是否正确”。图~\ref{fig:extra_offset_force}--\ref{fig:extra_offset_moment} 给出了偏心工况下的力与力矩对比，图~\ref{fig:extra_offset_geom} 展示了偏心 footprint 与局部二维积分点的几何恢复。代表性误差列于表~\ref{tab:extra_offset}。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{additional_validation_offset_geometry.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{additional_validation_offset_force.png}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{additional_validation_offset_moment.png}
    \end{minipage}
    \caption{偏心浅嵌入工况下的额外验证。左：竖直合力 $F_y$；右：偏心力矩 $M_x,M_z$。最终推荐算法 $B_{\mathrm{sdf\_quad\_sheet}}$ 同时保持了极低的力误差与力矩误差。}
    \label{fig:extra_offset_force}
    \label{fig:extra_offset_moment}
\end{figure}
...
\end{equation}
这组实验检验的是：当接触片曲率发生明显变化时，最终推荐离散是否仍然稳定。图~\ref{fig:extra_curvature} 给出了相对误差随球半径的变化。可以看到，$A_{\mathrm{direct\_band}}$ 的误差整体维持在 $10^{-1}$ 量级，而 $B_{\mathrm{sdf\_column\_center}}$ 已经可以把误差压到 $10^{-3}$ 到 $10^{-4}$ 之间；$B_{\mathrm{sdf\_quad\_sheet}}$ 则稳定保持在接近 $10^{-6}$ 的量级。也就是说，最终推荐算法并不是“对某个固定球半径调出来的”，而是对曲率变化本身有鲁棒性。
...
\begin{equation}
    k\in\{2,\,5,\,10,\,20,\,40\}.
\end{equation}
由于解析力严格按 $k$ 线性缩放，这组实验等价于检验离散算法是否保留了正确的刚度标度。图~\ref{fig:extra_stiffness} 表明：$A_{\mathrm{direct\_band}}$ 的相对误差几乎不随 $k$ 改变，维持在大约 $1.34\times 10^{-1}$；$B_{\mathrm{sdf\_column\_center}}$ 维持在大约 $1.79\times 10^{-3}$；$B_{\mathrm{sdf\_quad\_sheet}}$ 则始终保持在约 $3\times 10^{-6}$。因此，最终推荐算法不仅“几何上准”，而且“材料标度上也准”。

\begin{figure}[H]
    \centering
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{additional_validation_curvature_error.png}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{additional_validation_stiffness_error.png}
    \end{minipage}
    \caption{额外验证中的参数扫描。左：曲率扫描；右：刚度扫描。$B_{\mathrm{sdf\_quad\_sheet}}$ 在两类扫描中都显著优于直接厚带积分，也优于单列中心版本。}
    \label{fig:extra_curvature}
    \label{fig:extra_stiffness}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{additional_validation_convergence_error.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{requested_shape_examples_geometry_demo.png}
...
图~\ref{fig:user_small_cube_force}--\ref{fig:user_cylinder_force} 给出了三类算例各自的力--位移曲线，图~\ref{fig:user_conv} 给出了三类算例在 $\delta=0.03$ 时的收敛图。可以看到，对于小立方体和横放圆柱，$B_{\mathrm{local\_normal\_quad}}$ 在非常粗的二维分辨率下就已接近解析真值；环形冲头由于 footprint 带孔，对数值实现更苛刻，但最终算法依然稳定维持在约 $10^{-3}$ 量级，而直接厚带积分仍停留在 $10^{-1}$ 量级附近。

\begin{figure}[H]
    \centering
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{requested_small_cube_force_compare.png}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{requested_annular_punch_force_compare.png}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{requested_horizontal_cylinder_force_compare.png}
    \end{minipage}
    \caption{用户指定三类浅嵌入算例的力--位移对比。左：小立方体；中：环形平底冲头；右：横放有限长圆柱。}
    \label{fig:user_small_cube_force}
    \label{fig:user_annulus_force}
    \label{fig:user_cylinder_force}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{requested_shape_examples_convergence.png}
