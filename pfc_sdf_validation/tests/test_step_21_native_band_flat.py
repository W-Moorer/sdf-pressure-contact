...
def build_flat_overlap_fields(*, overlap: float, k_a: float, k_b: float):
    dx = dy = 0.05
    dz = 0.01
    nx, ny, nz = 40, 24, 81
    grid = UniformGrid3D(
...
def test_sdf_native_band_recovers_flat_force_and_measure():
    overlap = 0.12
    k_a = 14.0
    k_b = 21.0
    grid, fields = build_flat_overlap_fields(overlap=overlap, k_a=k_a, k_b=k_b)
...
def test_sdf_native_band_recovers_offset_torque_for_flat_contact():
    overlap = 0.08
    k_a = 10.0
    k_b = 15.0
    grid, fields = build_flat_overlap_fields(overlap=overlap, k_a=k_a, k_b=k_b)
    ...
    result = accumulate_sdf_native_band_wrench(fields, config, reference=reference)
