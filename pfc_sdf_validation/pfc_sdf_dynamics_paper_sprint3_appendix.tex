...
本文的 Sprint 1 贡献不再停留在单个 benchmark 或散乱脚本层面，而是形成了可复现、可导表、可导图的实验基础设施。项目现在包含统一的配置文件、主表脚本、效率表脚本、图生成脚本和导表工具，能够一键生成长时程 flat/sphere/native-band 主表、效率表以及论文主图，并进一步补入一组 6-DoF complex benchmark 的正文证据。现有结果支持以下阶段性结论：第一，SDF-native 主链已经完整打通；第二，continuity-aware traversal 在不明显带偏主结果的前提下显著降低了 traversal 成本；第三，时间一致性的两轮改进链（impulse correction 与 work consistency）在长时程 flat/sphere benchmark 中系统改善了冲量、能量与释放阶段的动力学指标；第四，在 6-DoF 复杂刚体接触中，rigid-motion-aware continuity 与 wrench-aware controller 共同支撑了位置/姿态/力/力矩误差的稳定控制。与此同时，本文也保持审慎：尽管当前已经补入一组复杂动态场景外推证据，更一般三维复杂几何上的长时程证据链仍需继续补强。
\end{abstract}
...
\begin{enumerate}
    ...
    \item 完成了 \textbf{Sprint 1 投稿闭环}：以统一脚本和配置自动生成长时程主表、效率表与论文主图，并将 6-DoF complex benchmark 的正文消融、效率与参考对比表一并纳入论文证据链，为后续补强偏心 flat、多 patch、reference comparison 与 sensitivity study 打下了可复现基础。
\end{enumerate}
...
\section{结果与分析}
\subsection{长时程动力学主表}
\input{results/tables/main_ablation.tex}
...
\input{results/tables/efficiency_continuity.tex}
\input{results/tables/efficiency_traction.tex}
...
\input{results/tables/complex6dof_ablation.tex}
...
\input{results/tables/complex6dof_efficiency.tex}
\input{results/tables/complex6dof_reference_comparison.tex}

表~\ref{tab:complex6dof_efficiency} 与表~\ref{tab:complex6dof_reference_comparison} 则补全了这组外推证据的另两个维度。第一，在关闭 controller 的低成本模式下，rigid-motion-aware continuity 仍能把 mean candidate cells 从 599.87 降到 546.61，把 mean recompute cells 从 531.22 降到 467.00，说明 continuity 在更便宜但更粗糙的 operating mode 下仍然保有明确工程收益。第二，完整方法相对于高分辨率同 scheme 参考轨迹的 peak force error 与 peak torque error 仅为 $1.139\times 10^{-4}$ 和 $7.223\times 10^{-4}$，说明当前 6-DoF 主链的主要误差已经不再表现为接触释放时刻或拓扑个数的灾难性偏差，而更多集中在连续 force/torque 历史的细部逼近上。结合前三组长时程 benchmark，这组 6-DoF 结果使得本文关于“复杂动态场景外推证据”的论证从平面法向接触进一步扩展到了分布式 wrench 与刚体姿态耦合层面。
...
\input{results/tables/eccentric_flat.tex}
...
\input{results/tables/multipatch_dynamic.tex}
...
\input{results/tables/controller_statistics.tex}
...
\input{results/tables/reference_comparison.tex}
...
\input{results/tables/sensitivity.tex}
